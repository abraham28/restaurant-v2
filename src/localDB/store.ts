 ...
 *    await storeData('form_data', formDataObject);
 *    const formData = await getData<FormData>('form_data');
 *
...
/**
 * ============================================================================
 * FORM DATA FUNCTIONS - Generic form persistence helpers
 * ============================================================================
 */
 ...
 * @param key - Optional custom storage key (defaults to 'form_data')
 * @returns Promise that resolves when data is stored
 *
 * @example
 * await storeFormData(formDataObject);
 * await storeFormData(formDataObject, 'my_custom_form_key');
 */
export async function storeFormData<T>(
  formData: T,
  key: string = 'form_data',
): Promise<void> {
  return storeData(key, formData);
}
 ...
 *
 * @example
 * const formData = await getFormData<FormData>();
 * const formData = await getFormData<FormData>('my_custom_form_key');
 */
 ...
 * await removeFormData('my_custom_form_key');
 */
export async function removeFormData(key: string = 'form_data'): Promise<void> {
  return removeData(key);
}

/**
 * @deprecated Use storeFormData instead. This function is kept for backward compatibility.
 */
export async function storeClientFormData<T>(formData: T): Promise<void> {
  return storeFormData(formData);
}

/**
 * @deprecated Use getFormData instead. This function is kept for backward compatibility.
 */
...
/**
 * @deprecated Use removeFormData instead. This function is kept for backward compatibility.
 */
