 ...
 *
 * @example
 * await storeCache('csv_data', parsedData);
 * await storeCache('json_data', jsonData);
 */
 ...
 *
 * @example
 * const cached = await getCache<Record<string, string>[]>('csv_data');
 * const jsonData = await getCache<MyType[]>('json_data');
 */
 ...
 *
 * @example
 * await removeCache('csv_data');
 * await removeCache('json_data');
 */
