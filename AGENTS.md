# Core Banking FE - Coding Standards

## Project Structure

### Folder Organization
- **Pages**: Each page/feature has its own folder under `src/pages/`
  - **Page folder naming**: Must use PascalCase (e.g., `Home`, `ClientInformationSystem`, `Dashboard`, `SamplePage`)
  - Main component file: `index.tsx` (e.g., `src/pages/ClientInformationSystem/index.tsx`)
  - **Index files**: Must only contain `export { default } from "./ComponentName";` (e.g., `export { default } from "./ClientInformationSystem";`)
  - Component file: Separate file with PascalCase name (e.g., `ClientInformationSystem.tsx`, `ClientInformationSystemInsert.tsx`)
  - **Sub-components/Tabs**: Each component or tab within a page should be in its own folder (e.g., `Individual/`, `Contacts/`, `Documents/`, `Employment/`)
  ...
  - Styles: Component-specific SCSS module (e.g., `ClientInformationSystem.module.scss`)
...
- Pages: PascalCase folder names (e.g., `ClientInformationSystem/`, `Home/`, `Dashboard/`)
...
- Use nested route objects for sub-routes (e.g., `ROUTES.CLIENT_INFORMATION_SYSTEM.INSERT`)
...
└── ClientInformationSystem/
    ├── ClientInformationSystem.tsx
    ├── index.tsx          # export { default } from './ClientInformationSystem';
    ├── ClientInformationSystem.module.scss
    ├── Individual/
    │   ├── Individual.tsx
    │   ├── index.tsx      # export { default } from './Individual';
    │   └── types.ts       # IndividualProps interface (if needed)
    └── Contacts/
        ├── Contacts.tsx
        ├── index.tsx      # export { default } from './Contacts';
        └── types.ts       # ContactsProps interface (if needed)
```
